...
public class DatasetValidationService
{
    ...
    private (bool hasInvalidValues, int invalidCount) ValidateNumericFeatures(Cicids2017TrainingData row)
    {
        ...
        {
            row.DestinationPort, row.FlowDuration, row.TotalFwdPackets, row.TotalBackwardPackets,
            row.TotalLengthOfFwdPackets, row.TotalLengthOfBwdPackets, row.FwdPacketLengthMax,
            row.FwdPacketLengthMin, row.FwdPacketLengthMean, row.FwdPacketLengthStd,
            row.BwdPacketLengthMax, row.BwdPacketLengthMin, row.BwdPacketLengthMean,
            row.BwdPacketLengthStd, row.FlowBytesPerSecond, row.FlowPacketsPerSecond,
            row.FlowIATMean, row.FlowIATStd, row.FlowIATMax, row.FlowIATMin,
            row.FwdIATTotal, row.FwdIATMean, row.FwdIATStd, row.FwdIATMax, row.FwdIATMin,
            row.BwdIATTotal, row.BwdIATMean, row.BwdIATStd, row.BwdIATMax, row.BwdIATMin,
            row.FwdPSHFlags, row.BwdPSHFlags, row.FwdURGFlags, row.BwdURGFlags,
            row.FINFlagCount, row.SYNFlagCount, row.RSTFlagCount, row.PSHFlagCount,
            row.ACKFlagCount, row.URGFlagCount, row.CWEFlagCount, row.ECEFlagCount,
            row.FwdHeaderLength, row.BwdHeaderLength, row.FwdPacketsPerSecond,
            row.BwdPacketsPerSecond, row.MinPacketLength, row.MaxPacketLength,
            row.PacketLengthMean, row.PacketLengthStd, row.PacketLengthVariance,
            row.DownUpRatio, row.AveragePacketSize, row.AvgFwdSegmentSize,
            row.AvgBwdSegmentSize, row.FwdAvgBytesPerBulk,
            row.FwdAvgPacketsPerBulk, row.FwdAvgBulkRate, row.BwdAvgBytesPerBulk,
            row.BwdAvgPacketsPerBulk, row.BwdAvgBulkRate, row.SubflowFwdPackets,
            row.SubflowFwdBytes, row.SubflowBwdPackets, row.SubflowBwdBytes,
            row.InitWinBytesForward, row.InitWinBytesBackward, row.ActiveMean,
            row.ActiveStd, row.ActiveMax, row.ActiveMin, row.IdleMean,
            row.IdleStd, row.IdleMax, row.IdleMin, row.ActDataPktFwd,
            row.MinSegSizeForward, row.Protocol
        };
